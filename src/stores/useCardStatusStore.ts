...
interface CardStatusProps{
    ...
    selectCard: (index: number, number: number) => void;
...
export const useCardStatusStore = create<CardStatusProps>(set=>({
    ...
    selectCard : (index, number) => {
        set(state => {
            const copyArray = [...state.cards_opend];
            copyArray[index] = true;
            return {cards_opend: copyArray}})
    }
